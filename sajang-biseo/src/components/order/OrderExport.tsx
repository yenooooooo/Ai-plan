...
export function OrderExport({ confirmedItems, itemsMap, orderDate }: OrderExportProps) {
  ...
  function handleDownload(blob?: Blob | null) {
    if (!blob) {
      generateImage().then((b) => { if (b) downloadBlob(b); });
      return;
    }
    downloadBlob(blob);
  }
          ...
          >
            ...
            >
              ...
              <div className="flex justify-end p-3">
                <button onClick={() => setShowPreview(false)} className="p-1 text-neutral-400">
                  <X size={20} />
                </button>
              </div>
                ...
                {groups.map((g) => (
                  <div key={g.supplierName} className="mb-3">
                    <p className="text-sm font-semibold text-neutral-800 mb-1">
                      {g.supplierName}
                      {g.supplierContact && (
                        <span className="font-normal text-neutral-400 ml-2">{g.supplierContact}</span>
                      )}
                    </p>
                    {g.items.map((item, i) => (
