...
import { Phone, MessageCircle, ChevronDown, Building2, Users, Search } from "lucide-react";
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="glass-card p-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="text-body-small font-semibold text-[var(--text-primary)]">거래처 관리</h3>
      ...
      <div className="relative mb-3">
        ...
        <input type="text" value={search} onChange={(e) => setSearch(e.target.value)}
          placeholder="거래처 또는 품목 검색..."
          className="w-full pl-8 pr-3 py-2 rounded-xl bg-[var(--bg-tertiary)] text-caption text-[var(--text-primary)] placeholder:text-[var(--text-tertiary)] outline-none" />
      </div>

      {filtered.length === 0 ? (
        <p className="text-center text-caption text-[var(--text-tertiary)] py-4">검색 결과가 없습니다</p>
      ) : (
          ...
          {filtered.map((supplier) => {
            ...
            return (
              <div key={supplier.name} className="bg-[var(--bg-tertiary)] rounded-xl overflow-hidden">
                <button onClick={() => setExpandedSupplier(isExpanded ? null : supplier.name)}
                  ...
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <span className="text-body-small font-medium text-[var(--text-primary)] truncate">{supplier.name}</span>
                    <span className="text-caption text-[var(--text-tertiary)] shrink-0">{supplier.items.length}개 품목</span>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    {supplier.contact && (
                      <div className="flex gap-1">
                        ...
                        <a href={`sms:${supplier.contact}`} onClick={(e) => e.stopPropagation()}
                        ...
                        {supplier.items.map((item) => (
                          <div key={item.id} className="flex items-center justify-between py-1">
                            ...
                            <span className="text-caption text-[var(--text-tertiary)]">
                              {item.unit}{item.unitPrice ? ` · ₩${item.unitPrice.toLocaleString()}` : ""}
                            </span>
                          </div>
                        ))}
                      </div>
