...
import { Calendar, ChevronDown, ChevronUp, RotateCcw } from "lucide-react";
...
interface OrderHistoryProps {
  items: DBOrderItem[];
  onReorder?: (entries: { itemId: string; qty: number }[]) => void;
}
...
export function OrderHistory({ items, onReorder }: OrderHistoryProps) {
  ...
  const loadHistory = useCallback(async () => {
    ...
    try {
      ...
      if (!data || data.length === 0) { setRecords([]); setLoading(false); return; }
      ...
      for (const row of data) {
        ...
        if (existing) {
          ...
          byDate.set(row.date, { date: row.date, entries: [entry], totalCost: (entry.unitPrice ?? 0) * entry.qty });
        }
      }
  ...
  if (records.length === 0) {
    return (
      <div className="glass-card p-5 text-center">
        ...
        <p className="text-body-small text-[var(--text-tertiary)]">발주 이력이 없어요</p>
        <p className="text-caption text-[var(--text-tertiary)] mt-1">발주를 저장하면 여기에 이력이 쌓여요</p>
      </div>
    );
  }
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="glass-card p-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="text-body-small font-semibold text-[var(--text-primary)]">발주 이력 (최근 30일)</h3>
      </div>
        ...
        {records.map((record) => {
          ...
          return (
            <div key={record.date} className="bg-[var(--bg-tertiary)] rounded-xl overflow-hidden">
              <button onClick={() => setExpandedDate(isExpanded ? null : record.date)}
                className="w-full flex items-center justify-between p-3 press-effect">
                <div className="flex items-center gap-2">
                  <span className="text-body-small font-medium text-[var(--text-primary)]">{dateLabel}</span>
                  <span className="text-caption text-[var(--text-tertiary)]">{record.entries.length}개 품목</span>
                </div>
                <div className="flex items-center gap-2">
                  {onReorder && (
                    <button onClick={(e) => { e.stopPropagation(); onReorder(record.entries.map(en => ({ itemId: en.itemId, qty: en.qty }))); }}
                      className="px-2 py-1 rounded-lg text-[11px] font-medium text-primary-500 bg-primary-500/10 flex items-center gap-1 press-effect">
                      <RotateCcw size={10} />재주문
                    </button>
                  )}
                  {record.totalCost > 0 && (
                    ...
                    <div className="px-3 pb-3 space-y-1 border-t border-[var(--border-subtle)]">
                      {record.entries.map((entry, i) => (
                        <div key={i} className="flex items-center justify-between py-1.5">
                          ...
                          <div className="flex items-center gap-2">
                            <span className="text-caption font-display text-[var(--text-secondary)]">{entry.qty}{entry.unit}</span>
                            {entry.unitPrice && (
                              <span className="text-caption text-[var(--text-tertiary)]">{formatCurrency(entry.unitPrice * entry.qty)}</span>
                            )}
                          </div>
                        </div>
                      ))}
