  ...
  return adding ? (
    <div className="flex gap-2">
      <input
        ...
        placeholder="카테고리 이름"
        className="flex-1 h-10 px-3 rounded-xl bg-[var(--bg-tertiary)] text-body-small text-[var(--text-primary)]
...
export default function OrderPage() {
  ...
  const analyticsItemId = useMemo(
    () => selectedItemId ?? (activeItems[0]?.id ?? null),
    [selectedItemId, activeItems]
  );
  const analytics = useOrderAnalytics(items, analyticsItemId);
  const selectedItem = useMemo(
    () => items.find((i) => i.id === analyticsItemId),
    [items, analyticsItemId]
  );
      ...
      <AnimatePresence mode="wait">
        ...
        {tab === "usage" && (
          ...
          >
            ...
            {/* 저장 버튼 */}
            <motion.button whileTap={{ scale: 0.97 }} onClick={saveUsage} disabled={usageSaving || (!hasUsageData && !usageSaved)} className={`w-full py-4 rounded-2xl font-semibold text-body-small flex items-center justify-center gap-2 press-effect ${usageSaved ? "bg-success/10 text-success" : "bg-primary-500 text-white"} disabled:opacity-50`}>
              {usageSaving ? (
            ...
            {recLoading ? (
              ...
              <div className="glass-card p-8 text-center">
                <p className="text-body-small text-[var(--text-tertiary)]">
                  사용량 데이터가 부족합니다.<br />일일 사용량을 먼저 입력해주세요.
                </p>
              </div>
            ) : (
              <>
          ...
          <motion.div key="settings" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} transition={{ duration: 0.2 }} className="space-y-3">
            {loading ? (
              ...
              <div className="glass-card p-8 text-center space-y-4">
                ...
                <AddGroupInline
                  adding={addingGroup} name={newGroupName}
